...
The delete path is now internally consistent across the relay:

\begin{itemize}[leftmargin=1.5cm]
    \item \texttt{main.zig} dispatches requests whose path begins with \texttt{/delete/}.
    \item \texttt{delete.zig} extracts the blob identifier using the same \texttt{/delete/} prefix.
\end{itemize}

As a result, a normal request of the form \texttt{/delete/\{id\}?token=...} reaches the handler, parses the identifier correctly, validates the token, and removes both the blob and its metadata when authorized.
...
The test vectors in these files follow RFC 8439 style reference cases, and the implementation was validated against them successfully.
...
\begin{tikzpicture}[
    node distance=1.9cm,
    box/.style={draw, rounded corners, minimum width=5.2cm, minimum height=0.95cm, text width=5.2cm, inner sep=7pt, align=center, font=\small},
    sidebox/.style={draw, rounded corners, minimum width=5.8cm, minimum height=1.25cm, text width=5.8cm, inner sep=8pt, align=center, font=\small},
    arr/.style={->, thick, >=stealth, rounded corners=4pt},
    ...
    \node[sidebox, fill=red!10, right of=append, xshift=6.0cm, yshift=-1.1cm] (reap) {Background reaper deletes expired complete uploads and abandoned partial uploads};

    \draw[arr] (start) -- (append);
    \draw[arr] (append) -- (finish);
    \draw[arr] (finish) -- (download);

    \path (start.east) ++(1.1,0) coordinate (busTop);
    \path (append.east) ++(1.1,0) coordinate (busMid);
    \path (finish.east) ++(1.1,0) coordinate (busBot);
    \path (reap.west) ++(-0.65,0) coordinate (reapIn);

    \draw[link] (start.east) -- (busTop) -- (busMid);
    \draw[link] (append.east) -- (busMid);
    \draw[link] (finish.east) -- (busBot) -- (busMid);
    \draw[arr] (busMid) -- (reapIn) -- (reap.west);
\end{tikzpicture}
\end{center}
...
The relay already has the core properties needed for a working product prototype: bounded uploads, per-IP limits, upload resumption safety through index checking, background expiry, and simple token-based authorization for upload continuation and deletion. The main areas that still deserve attention are route consistency, download streaming efficiency, and a careful review of protocol nonce handling in the client/format layer.
